...
export class ChartEngine {
    ...
    setTimeframes(timeframes) {
        this.timeframes = timeframes.sort((a, b) => this.parseTimeframeToSeconds(a) - this.parseTimeframeToSeconds(b));
    ...
    updateCrosshair() {
        if (!this.state.showCrosshair || this.state.data.length === 0)
            return;
        const rect = this.overlayCanvas.getBoundingClientRect();
        const chartWidth = rect.width - this.layout.marginLeft - this.layout.marginRight;
        ...
        for (const candle of this.state.data) {
            if (candle.time < this.state.viewStart || candle.time > this.state.viewEnd)
                continue;
    ...
    async loadIndicatorData() {
        ...
        for (const tf of rsiTimeframes) {
            try {
                ...
                const data = await this.callbacks.onLoadData(this.state.symbol, tf, Math.floor(this.state.viewStart - margin), Math.ceil(this.state.viewEnd + margin));
    ...
    updateRSILegend() {
        ...
        if (this.rsiData.size === 0 || !chartConfig.get('indicators.enabled'))
            return;
    ...
    getRSIColorForTimeframe(tf) {
        // Gradient de chaleur: TF fine → rouge/orange, TF large → jaune
        const intensity = chartConfig.get('indicators.rsi.heatIntensity') || 1.0;
        const tfIndex = this.timeframes.indexOf(tf);
        if (tfIndex === -1)
            return '#FF9800';
        // Normaliser l'index (0 = TF la plus fine, 1 = TF la plus large)
        const normalizedIndex = this.timeframes.length > 1
            ? tfIndex / (this.timeframes.length - 1)
            : 0.5;
        // Interpoler entre rouge (0) et jaune (1)
        // Rouge: #FF4500, Orange: #FF8C00, Jaune: #FFD700
        const r = 255;
        const g = Math.round(69 + (215 - 69) * normalizedIndex * intensity);
        const b = Math.round(0 + (0 + 140 * normalizedIndex) * intensity);
    ...
    render() {
        ...
        const visibleCandles = this.state.data.filter(c => c.time >= this.state.viewStart && c.time <= this.state.viewEnd);
    ...
    renderDynamicOverlay() {
        ...
        let dashArray = [];
        if (crosshairStyle === 'dashed')
            dashArray = [4, 4];
        else if (crosshairStyle === 'dotted')
            dashArray = [2, 2];
        // Crosshair lines
    ...
    renderTooltip(candle) {
        ...
        if (x + tooltipW > rect.width)
            x = this.state.mouseX - tooltipW - 15;
        if (y + tooltipH > rect.height)
            y = this.state.mouseY - tooltipH - 15;
        // Fond
